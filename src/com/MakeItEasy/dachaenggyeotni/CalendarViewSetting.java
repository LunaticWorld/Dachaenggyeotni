		...
		for (int i = 1; i <= maxRow; i++) {
			...
			for (int j = 1; j <= maxColumn; j++) {
				...
				if (dayCnt >= 0 && dayCnt < 7) {
					// oneday[dayCnt].setBgDayPaint(Color.BLACK); // 배경색상
					// --------------------------------------
					oneday[dayCnt].setTextDayTopPadding(12); // 일자표시 할때 top
																// padding
					// oneday[dayCnt].setTextDayColor(Color.WHITE); // 일자의 글씨 색상
					// --------------------------------
					oneday[dayCnt].setTextDaySize(50); // 일자의 글씨크기
					oneday[dayCnt].setLayoutParams(new LayoutParams(
							oneday_width, 100)); // 일자 컨트롤 크기
